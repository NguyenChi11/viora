...
if (!function_exists('viora_home_client_default_data')) {
    function viora_home_client_default_data()
    {
        return array(
            'enabled' => true,
            'heading' => array(
                'kicker' => __('Loved by teams worldwide', 'viora'),
                'title' => __('Client Success Stories', 'viora'),
                'lede' => __('Trusted by founders, product teams, and global brands to launch premium digital experiences.', 'viora'),
            ),
            'testimonials' => array(),
        );
    }
...
if (!function_exists('viora_home_client_has_testimonials')) {
    function viora_home_client_has_testimonials($data)
    {
        if (function_exists('viora_home_client_normalize_input')) {
            $data = viora_home_client_normalize_input($data);
        }
        ...
        $items = isset($data['testimonials']) && is_array($data['testimonials'])
            ? $data['testimonials']
            : array();
        ...
        foreach ($items as $item) {
            ...
            if (isset($item['avatar_url']) && is_string($item['avatar_url']) && trim($item['avatar_url']) !== '') {
                $avatar = trim($item['avatar_url']);
            } elseif (isset($item['avatar']) && is_string($item['avatar']) && trim($item['avatar']) !== '') {
...
if (!function_exists('viora_home_client_normalize_input')) {
    function viora_home_client_normalize_input($value)
    {
        if (is_string($value)) {
            $decoded = json_decode(wp_unslash($value), true);
            if (is_array($decoded)) {
        ...
        if (is_array($value)) {
            foreach ($value as $key => $item) {
                $value[$key] = viora_home_client_normalize_input($item);
            }
        }
...
if (!function_exists('viora_home_client_sanitize_data')) {
    function viora_home_client_sanitize_data($value)
    {
        if (function_exists('viora_home_client_normalize_input')) {
            $value = viora_home_client_normalize_input($value);
        }
        ...
        $defaults = function_exists('viora_home_client_default_data')
            ? viora_home_client_default_data()
            : array();
        ...
        $default_heading = isset($defaults['heading']) && is_array($defaults['heading']) ? $defaults['heading'] : array();
        $heading = isset($value['heading']) && is_array($value['heading']) ? $value['heading'] : array();

        $testimonials_raw = array();
        if ($is_list($value)) {
            $testimonials_raw = $value;
        } elseif (isset($value['testimonials']) && is_array($value['testimonials'])) {
            $testimonials_raw = $value['testimonials'];
        }

        $testimonials = array();
        foreach ($testimonials_raw as $item) {
            if (!is_array($item)) {
                continue;
            }

            $avatar_url = esc_url_raw($pick($item, array('avatar_url', 'avatar'), ''));
            $sanitized_item = array(
                'quote' => sanitize_textarea_field(isset($item['quote']) ? $item['quote'] : ''),
                'name' => sanitize_text_field(isset($item['name']) ? $item['name'] : ''),
                'role' => sanitize_text_field(isset($item['role']) ? $item['role'] : ''),
                'avatar_id' => absint(isset($item['avatar_id']) ? $item['avatar_id'] : 0),
                'avatar_url' => $avatar_url,
            );
            $sanitized_item['avatar'] = $avatar_url;

            if ($sanitized_item['quote'] !== '' || $sanitized_item['name'] !== '' || $sanitized_item['role'] !== '' || $sanitized_item['avatar_url'] !== '') {
                $testimonials[] = $sanitized_item;
            }
        }
        ...
        return array(
            'enabled' => isset($value['enabled']) ? (bool) $value['enabled'] : true,
            'heading' => array(
                'kicker' => sanitize_text_field(isset($heading['kicker']) ? $heading['kicker'] : (isset($default_heading['kicker']) ? $default_heading['kicker'] : '')),
                'title' => sanitize_text_field(isset($heading['title']) ? $heading['title'] : (isset($default_heading['title']) ? $default_heading['title'] : '')),
                'lede' => sanitize_textarea_field(isset($heading['lede']) ? $heading['lede'] : (isset($default_heading['lede']) ? $default_heading['lede'] : '')),
            ),
...
if (!function_exists('viora_home_client_deep_merge')) {
    function viora_home_client_deep_merge($base, $patch)
    {
        $base = is_array($base) ? $base : array();
        $patch = is_array($patch) ? $patch : array();
        ...
        foreach ($patch as $key => $patch_value) {
            if (is_array($patch_value)) {
                ...
                $base_value = isset($base[$key]) && is_array($base[$key]) ? $base[$key] : array();
                $base[$key] = viora_home_client_deep_merge($base_value, $patch_value);
                continue;
            }
...
if (!function_exists('viora_home_client_merge_with_current')) {
    function viora_home_client_merge_with_current($incoming, $page_id = 0)
    {
        if (function_exists('viora_home_client_normalize_input')) {
            $incoming = viora_home_client_normalize_input($incoming);
        }

        $incoming = is_array($incoming) ? $incoming : array();

        $current = get_theme_mod('viora_home_client_data', array());
        if (function_exists('viora_home_client_normalize_input')) {
            $current = viora_home_client_normalize_input($current);
        }
        if (!is_array($current) || empty($current)) {
        ...
        return viora_home_client_deep_merge($current, $incoming);
    }
}

if (!function_exists('viora_home_client_get_default_data')) {
    function viora_home_client_get_default_data()
    {
        return viora_home_client_sanitize_data(array());
    }
}

if (!function_exists('viora_home_client_is_home_page_id')) {
    function viora_home_client_is_home_page_id($post_id)
    {
        if (function_exists('viora_home_banner_is_home_page_id')) {
            return viora_home_banner_is_home_page_id($post_id);
        }
...
if (!function_exists('viora_home_client_find_home_page_id')) {
    function viora_home_client_find_home_page_id()
    {
        if (function_exists('viora_home_banner_find_home_page_id')) {
            return viora_home_banner_find_home_page_id();
        }
        ...
        $front_id = (int) get_option('page_on_front');
        if ($front_id > 0 && viora_home_client_is_home_page_id($front_id)) {
            return $front_id;
        }
...
if (!function_exists('viora_home_client_collect_target_page_ids')) {
    function viora_home_client_collect_target_page_ids($preferred_page_id = 0)
    {
        if (function_exists('viora_home_banner_collect_target_page_ids')) {
            return viora_home_banner_collect_target_page_ids($preferred_page_id);
        }
        ...
        $targets = array();
        $preferred_page_id = absint($preferred_page_id);
        if ($preferred_page_id > 0 && viora_home_client_is_home_page_id($preferred_page_id)) {
            $targets[] = $preferred_page_id;
        }
        ...
        $targets = array_unique(array_filter(array_map('absint', $targets)));
        if (empty($targets)) {
            $fallback_id = viora_home_client_find_home_page_id();
            if ($fallback_id > 0) {
                $targets[] = $fallback_id;
            }
        }
...
if (!function_exists('viora_home_client_sync_storage')) {
    function viora_home_client_sync_storage($enabled, $data, $preferred_page_id = 0)
    {
        $enabled = absint($enabled) === 1 ? 1 : 0;
        $data = viora_home_client_sanitize_data($data);
        $data['enabled'] = ($enabled === 1);

        $targets = viora_home_client_collect_target_page_ids($preferred_page_id);
        $preferred_page_id = absint($preferred_page_id);
        if (empty($targets) && $preferred_page_id > 0) {
            $targets[] = $preferred_page_id;
        }
        ...
        foreach ($targets as $target_id) {
            update_post_meta($target_id, 'viora_home_client_data', $data);
            update_post_meta($target_id, 'viora_home_client_enabled', $enabled);
            clean_post_cache($target_id);
        }
...
if (!function_exists('viora_home_client_get_data_for_front')) {
    function viora_home_client_get_data_for_front($page_id = 0)
    {
        $page_id = absint($page_id);
        if ($page_id <= 0) {
            $queried_id = get_queried_object_id();
        ...
        if ($page_id <= 0) {
            $page_id = viora_home_client_find_home_page_id();
        }

        $data = array();
        $enabled = null;
        if ($page_id > 0) {
            $meta_data = get_post_meta($page_id, 'viora_home_client_data', true);
            if (is_array($meta_data)) {
                $data = $meta_data;
            }

            $meta_enabled = get_post_meta($page_id, 'viora_home_client_enabled', true);
            if ($meta_enabled !== '') {
                $enabled = absint($meta_enabled);
            }
        }

        $has_items = function_exists('viora_home_client_has_testimonials')
            ? viora_home_client_has_testimonials($data)
            : (is_array($data) && !empty($data));

        if (!$has_items) {
            $mod_data = get_theme_mod('viora_home_client_data', array());
            if (function_exists('viora_home_client_normalize_input')) {
                $mod_data = viora_home_client_normalize_input($mod_data);
            }
            if (
                is_array($mod_data)
                && !empty($mod_data)
                && (
                    ...
                    || viora_home_client_has_testimonials($mod_data)
                )
            ) {
                $data = $mod_data;
                $has_items = true;
        ...
        if (!$has_items && function_exists('viora_import_parse_js')) {
        ...
        if ($enabled === null) {
            $enabled = absint(get_theme_mod('viora_home_client_enabled', isset($data['enabled']) ? (int) $data['enabled'] : 1));
        }

        if (is_customize_preview()) {
            $preview_data = get_theme_mod('viora_home_client_data', array());
            if (function_exists('viora_home_client_normalize_input')) {
                $preview_data = viora_home_client_normalize_input($preview_data);
            }
            if (is_array($preview_data) && !empty($preview_data)) {
                $data = $preview_data;
            }
            $enabled = absint(get_theme_mod('viora_home_client_enabled', $enabled));
        }
