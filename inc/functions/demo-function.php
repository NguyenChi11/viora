...
function viora_maybe_import_home_client_demo_once()
{
    if (get_option('viora_home_client_demo_imported') === '1') {
        return;
    }
    ...
    if (is_array($existing) && !empty($existing)) {
        update_option('viora_home_client_demo_imported', '1');
        return;
    }

    $file = get_theme_file_path('/import/data-demo/page/home/client.php');
    if (file_exists($file)) {
        require_once $file;
        if (function_exists('viora_import_home_client_demo')) {
            viora_import_home_client_demo();
        }
    }

    $after = get_post_meta($home_id, 'viora_home_client_data', true);
    if (is_array($after) && !empty($after)) {
        update_option('viora_home_client_demo_imported', '1');
    }
}

function viora_admin_maybe_import_home_client_demo_once()
{
    if (!is_admin()) {
        return;
    }
    ...
    viora_maybe_import_home_client_demo_once();
}
add_action('current_screen', 'viora_admin_maybe_import_home_client_demo_once');

add_action('after_switch_theme', 'viora_maybe_import_home_client_demo_once', 20);
